/** JSON responses shared by the live-data endpoints in /api/* (docs/koncept.md §9). */
...
/** Data for the private dashboard (docs/koncept.md §14): never cached anywhere. */
export function privateJson(data: object, status = 200): Response {
	return Response.json(data, { status, headers: { 'Cache-Control': 'private, no-store' } });
}

/**
 * Writes from the private dashboard take JSON only (docs/koncept.md §14): a
